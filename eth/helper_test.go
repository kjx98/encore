...
func newTestProtocolManagerConsensus(consensusAlgo string, cliqueConfig *params.CliqueConfig, raftMode bool) (*ProtocolManager, ethdb.Database, error) {
	...
	pm, err := NewProtocolManager(config, nil, 61, DefaultConfig.NetworkId, evmux, &testTxPool{added: nil}, engine, blockchain, db, 1, nil, raftMode)
	if err != nil {
		return nil, nil, err
	}
	pm.Start(1000)
