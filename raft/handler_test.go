...
func writeAppliedIndex(workingDir string, node int, index uint64) error {
	db, err := openEncoreRaftDb(fmt.Sprintf("%s/node%d/raft-state", workingDir, node+1))
	if err != nil {
		return err
	}
	defer func() {
...
func startRaftNode(id, port uint16, tmpWorkingDir string, key *ecdsa.PrivateKey, nodes []*enode.Node) (*RaftService, error) {
	...
	s, err := New(ctx, params.TestChainConfig, id, port, false, 100*time.Millisecond, e, nodes, datadir, false)
	if err != nil {
		return nil, err
	}
